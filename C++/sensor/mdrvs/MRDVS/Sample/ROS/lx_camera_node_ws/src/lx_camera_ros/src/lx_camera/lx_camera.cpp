...
LxCamera::LxCamera() {
    ...
    nh_->param<int>("is_xyzit", is_xyzit_, 0);
    ROS_INFO("publish depth: %d xyz: %d lidar: %d", is_depth_, is_xyz_, is_xyzit_);
    if (is_xyz_ || is_xyzit_ || is_depth_) {
...
int LxCamera::Start() {
    ...
    // get image params
    if (is_depth_ || is_amp_ || is_xyz_ || is_xyzit_) {
...
pcl::PointCloud<pcl::PointXYZRGB>::Ptr LxCamera::BuildPointCloudXYZRGB(
    float* xyz_data, uint8_t* rgb_data, long buff_len, int num_threads) {
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZRGB>);
    std::vector<std::vector<pcl::PointXYZRGB>> thread_local_clouds(num_threads);
    for (auto& vec : thread_local_clouds) {
        vec.reserve(buff_len / num_threads + 1);
    }
...
#pragma omp parallel for schedule(static) num_threads(num_threads)
    for (long i = 0; i < buff_len; i++) {
        ...
        thread_local_clouds[thread_id].emplace_back(point);
    }

    // 合并所有线程的结果
    size_t total_points = 0;
    for (const auto& vec : thread_local_clouds) {
        total_points += vec.size();
    }
    cloud->points.reserve(total_points);
    for (const auto& vec : thread_local_clouds) {
        cloud->points.insert(cloud->points.end(), vec.begin(), vec.end());
    }
...
pcl::PointCloud<pcl::PointXYZ>::Ptr LxCamera::BuildPointCloudXYZ(
    float* xyz_data, long buff_len, int num_threads) {
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZ>);

    std::vector<std::vector<pcl::PointXYZ>> thread_local_clouds(num_threads);
    for (auto& vec : thread_local_clouds) {
        vec.reserve(buff_len / num_threads + 1);
    }
...
#pragma omp parallel for schedule(static) num_threads(num_threads)
    for (long i = 0; i < buff_len; i++) {
        ...
        int thread_id = omp_get_thread_num();
        thread_local_clouds[thread_id].emplace_back(
            xyz_data[index],
            xyz_data[index + 1],
            xyz_data[index + 2]
        );
...
pcl::PointCloud<PointXYZIT>::Ptr LxCamera::BuildPointCloudXYZIT(
    const LxPointCloudData* data, int num_threads) {
    pcl::PointCloud<PointXYZIT>::Ptr cloud(new pcl::PointCloud<PointXYZIT>);
    uint32_t total_points = data->point_num;
    cloud->points.reserve(total_points);
    std::vector<std::vector<PointXYZIT>> thread_local_lidar_clouds(num_threads);
    for (auto& vec : thread_local_lidar_clouds) {
        vec.reserve(total_points / num_threads + 1);
    }
#pragma omp parallel for schedule(static) num_threads(num_threads)
    for (uint32_t i = 0; i < total_points; ++i) {
        int thread_lidar_id = omp_get_thread_num();
        const LxPointXYZIRT& point = data->points[i];
        double lidar_timestamp = static_cast<double>(data->timebase) + static_cast<double>(point.offset_time);
        thread_local_lidar_clouds[thread_lidar_id].emplace_back(
            point.x,
            point.y,
            point.z,
            point.intensity,
    ...
    for (const auto& vec : thread_local_lidar_clouds) {
        cloud->points.insert(cloud->points.end(), vec.begin(), vec.end());
    }
    cloud->width = cloud->points.size();
    cloud->height = 1;
...
void LxCamera::Run() {
    ...
    if ((is_xyz_ || is_xyzit_ || is_depth_ || is_amp_) && is_rgb_) {
    ...
    while (ros::ok()) {
        ...
        if (is_depth_ || is_xyz_ || is_xyzit_) {
            ...
            if (dep_data) {
                ...
                if (is_xyzit_) {
                    ...
                    if (DcGetPtrValue(handle_, LX_PTR_XYZIRT_DATA, (void**)&xyzirt_data) == LX_SUCCESS) {
                        ...
                        auto cloud = BuildPointCloudXYZIT(data, num_threads);
                    ...
                    if (DcGetPtrValue(handle_, LX_PTR_XYZ_DATA, (void**)&xyz_data) == LX_SUCCESS) {
                        ...
                        auto cloud = BuildPointCloudXYZRGB(xyz_data, rgb_data, buff_len, num_threads);
        ...
        if (is_amp_ || is_depth_ || is_xyz_ || is_xyzit_)
