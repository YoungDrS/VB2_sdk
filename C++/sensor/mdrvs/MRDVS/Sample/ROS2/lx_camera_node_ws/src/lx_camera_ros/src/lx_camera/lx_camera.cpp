...
LxCamera::LxCamera(DcLib *dynamic_lib) : Node("lx_camera_node") {
  ...
  this->get_parameter<it>("log_level", log_level_);
  ...
  this->get_parameter<it>("enable_gpu", enable_gpu);
...
#pragma omp parallel for schedule(static) num_threads(num_threads)
            ...
            for (const auto& vec : thread_local_lidar_clouds) {
                lidarCloud->points.insert(lidarCloud->points.end(), vec.begin(), vec.end());
            }
            ...
            //            std::string("Lidar data is empty!").c_str());
          }
        }
        if (is_xyz_) {
