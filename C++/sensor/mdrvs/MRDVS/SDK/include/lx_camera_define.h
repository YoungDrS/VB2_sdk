...
typedef enum JPEG_DECODE_METHOD 
{
    ...
    JPEG_DECODE_DP = 1,         //LxDataProcess，会优先加载本地的opencv_world
...
typedef struct LxPointXYZIRT {
    float x;             //lidar设备坐标系下的x坐标，单位m
    float y;             //lidar设备坐标系下的y坐标，单位m                         
    float z;             //lidar设备坐标系下的z坐标，单位m
...
}LxIntrinsicParameters;//相机内参的结构体
...
typedef enum LX_CAMERA_FEATURE
{
    ...
    LX_INT_2D_ENCODE_RATIO = 1055,      //默认2D图像以jpeg压缩，设置压缩比例。
                                        //[1-99]，值越小，压缩比越高，带宽越小，解码速度越快，但图像质量更差。再次StartStream后有效
    ...
    LX_INT_XYZ_UNIT         = 1079,     //点云单位，0：mm，1：m
