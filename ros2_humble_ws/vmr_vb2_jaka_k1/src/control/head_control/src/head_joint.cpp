...
#define DEV_INFO(...) \
  RCLCPP_INFO(rclcpp::get_logger("HeadSystemHardware"), __VA_ARGS__)

#define DEV_FATAL(...) \
  RCLCPP_INFO(rclcpp::get_logger("HeadSystemHardware"), __VA_ARGS__)

hardware_interface::CallbackReturn HeadSystemHardware::on_init(
...
hardware_interface::CallbackReturn HeadSystemHardware::read_configure(
    const hardware_interface::HardwareInfo& info) {
...
HeadSystemHardware::export_state_interfaces() {
...
HeadSystemHardware::export_command_interfaces() {
...
hardware_interface::CallbackReturn HeadSystemHardware::on_activate(
...
hardware_interface::CallbackReturn HeadSystemHardware::on_deactivate(
    const rclcpp_lifecycle::State& /*previous_state*/) {
  // BEGIN: This part here is for exemplary purposes - Please do not copy to
  // your production code
...
hardware_interface::return_type head_control::HeadSystemHardware::read(
  ...
  for (std::size_t pi = 0; pi < hw_positions_.size(); pi++) {
    hw_positions_[pi] = 3.1415926-fb_msg->data[pi];
    hw_velocities_[pi] = 0; // fb_msg->data[pi * 2 + 1];
  }
  return hardware_interface::return_type::OK;
}

hardware_interface::return_type head_control::HeadSystemHardware::write(
    const rclcpp::Time& /*time*/, const rclcpp::Duration& /*period*/) {
  if (cmd_.realtime_cmd_publisher_->trylock()) {
    ...
    for (auto i = 0u; i < hw_commands_.size(); i++) {
      cmd_msg.data[i] = 3.1415926-hw_commands_[i];
    }
...
PLUGINLIB_EXPORT_CLASS(head_control::HeadSystemHardware,
